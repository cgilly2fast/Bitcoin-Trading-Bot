...
export class SMA extends indicator.Indicator {
    ...
    //	Returns: 	list of Simple Moving Average data from time specifed to present 
    ...
    fromTime(time) {
        ...
        let startTime = parseInt(momentTime.format('X')); // to unix time
        // round down to nearest minute 
        if (startTime % 60 !== 0) {
            let base = startTime % 60;
            startTime += (60 - base);
        }
        // create end time to used in for loop
        let currentMinute = new Date().getTime() / 1000; // calculate prev minute closing to unix
        let smaResults;
    ...
    update() {
        ...
        let currentTime = parseInt(momentTime.format('X'));
        if (currentTime % 60 !== 0) {
            let base = currentTime % 60;
            currentTime -= base;
        }
        ...
        let startTime = currentTime - ((this.interval.toNumber() - 2) * 60);
        this.db.get("SELECT SUM(Close) FROM $exchangeProduct WHERE Timestamp >= $startTime AND Timestampe <= $endTime", {
            $exchageProduct: this.exchange.concat(this.product),
            $startTime: startTime,
            $endTime: currentTime
        }, (err, row) => {
            if (err) {
